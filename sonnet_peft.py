'''
[Variation 4: PEFT Only]
소네트 생성 모델 - PEFT Only 버전 (LoRA + CrossEntropy)

실행:
  `python sonnet_peft.py --epochs 1 --batch_size 4`
'''
...
def train(args):
  ...
  model = SonnetGPT(args, use_lora=True)
  model = model.to(device)

  # 오직 requires_grad=True인 LoRA 어댑터 가중치만 학습 진행
  ...
  for epoch in range(args.epochs):
    ...
    for batch in tqdm(sonnet_dataloader, desc=f'PEFT train-{epoch}', disable=TQDM_DISABLE):
    ...
    print(f"Epoch {epoch}: PEFT train loss :: {train_loss:.3f} | val loss :: {val_loss:.3f}")
...
def generate_submission_sonnets(args):
  ...
  print("\n=============================================")
  print("Final Evaluation on Fixed Metric Suite (PEFT ONLY)")
  print("=============================================")
...
if __name__ == "__main__":
  args = get_args()
  args.filepath = f'{args.epochs}-{args.lr}-sonnet-peft.pt'
