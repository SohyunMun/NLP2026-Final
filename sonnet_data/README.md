...
/home/msko021220/.conda/envs/busi2/bin/python evaluate_sonnet_metrics.py \
  ...
  --out_dir experiments/unified_sonnet_eval/MODEL_NAME_dev
```

## Notes

- `data/` 폴더는 원래 과제 repository의 전체 task 데이터를 보존한 위치이다.
- `sonnet_data/`는 sonnet generation 전용으로 다시 정리한 폴더이다.
- 새 실험에서는 가능하면 `sonnet_data/strict_497/` 경로를 사용한다.
