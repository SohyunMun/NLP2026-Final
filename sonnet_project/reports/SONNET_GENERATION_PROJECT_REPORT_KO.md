# Sonnet Generation: Main Training and Reranking Evaluation Report
...
| `poemetric_reranking` | best DPO checkpoint + reference-free reranking | dev/test prompts only | - | - | candidates 6, top_p/top_k decoding, POEMetric/form/rhyme/theme score |
...
| POEMetric reranking | 약 5-10분 |
| 전체 메인 workflow | 약 1-1.5시간 |
...
SFT는 chrF를 올리지는 못했지만 theme과 POEMetric을 개선했다. 특히 test POEMetric은 `0.6181`로 학습 모델 중 최고이다. SFT가 "앞 3줄 prompt를 받아 sonnet을 이어 쓰는 과제 형식"을 직접 학습했기 때문으로 볼 수 있다.
...
### 6.6 POEMetric reranking 효과

비교: `dapt_sft_lora_dpo_best_chrf` -> `POEMetric reranking`

| split | metric | DPO single | POEMetric reranking | 변화 |
|---|---|---:|---:|---:|
| dev | chrF | 42.7768 | 42.0672 | -0.7096 |
| dev | Sonnet-or-Not | 0.0000 | 0.1667 | +0.1667 |
| dev | form | 0.5613 | 0.5804 | +0.0191 |
| dev | theme | 0.2403 | 0.3307 | +0.0904 |
| dev | POEMetric | 0.5971 | 0.6359 | +0.0388 |
| test | Sonnet-or-Not | 0.0000 | 0.1667 | +0.1667 |
| test | form | 0.5571 | 0.5871 | +0.0300 |
| test | POEMetric | 0.6105 | 0.6496 | +0.0391 |

POEMetric reranking은 새 모델을 학습하지 않는 decoding/post-processing 설정이지만, 최종 제출물의 생성 후보를 결정하는 독립적인 방법이므로 메인 실험 설정에 포함했다. 이 설정은 dev chrF를 약간 낮췄지만, gold reference 없이 계산 가능한 형식, rhyme, theme, non-repetition 요소를 기준으로 후보를 고르기 때문에 dev/test 모두에서 Sonnet-or-Not과 POEMetric을 개선했다. 따라서 최종 제출 해석에서는 chrF 최고 설정은 DPO single, reference-free poetic quality 최고 설정은 POEMetric reranking으로 구분한다.
...
| dev/test POEMetric 최대화 | `poemetric_reranking` |
| test theme overlap 최대화 | `dapt_sft_lora_dpo_best_chrf` |
| test Sonnet-or-Not pass | `poemetric_reranking`, 단 12개 중 2개 통과 |
