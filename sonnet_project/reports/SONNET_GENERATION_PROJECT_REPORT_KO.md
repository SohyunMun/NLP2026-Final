...
POEMetric reranking은 dev chrF를 약간 낮췄지만, gold reference 없이 계산 가능한 형식, rhyme, theme, non-repetition 요소를 기준으로 후보를 고르기 때문에 dev/test 모두에서 Sonnet-or-Not과 POEMetric을 개선했다. 따라서 최종 제출 해석에서는 chrF 최고 모델은 DPO single, reference-free poetic quality 최고 결과는 POEMetric reranking으로 구분한다.
...
| 학습 모델 중 dev/test POEMetric 최대화 | `sft_plus_extra` |
| 후처리 포함 dev/test POEMetric 최대화 | `POEMetric reranking` |
| test theme overlap 최대화 | `dapt_sft_lora_dpo_best_chrf` |
| test Sonnet-or-Not pass | `POEMetric reranking`, 단 12개 중 2개 통과 |
