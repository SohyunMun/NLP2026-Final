#!/usr/bin/env python3
"""Run the requested sonnet generation ablation workflow.
...
7. poemetric_reranking: reference-free reranking on top of #6. This setting is
   generated by run_dpo_reranking.py and included in summaries when available.
...
DPO_RERANK_ROOT = PROJECT_ROOT / "experiments" / "dpo_reranking"
DPO_RERANK_DEV_SUMMARY = DPO_RERANK_ROOT / "eval" / "dev" / "dpo_reranked_summary_metrics.json"
DPO_RERANK_TEST_SUMMARY = DPO_RERANK_ROOT / "eval" / "test" / "dpo_reranked_summary_metrics.json"
...
def rows_with_poemetric_reranking(eval_path: Path, rerank_path: Path) -> list[dict[str, object]]:
    rows = load_rows(eval_path)
    rows = [row for row in rows if row.get("model_name") != "poemetric_reranking"]
    if rerank_path.exists():
        rerank_rows = load_rows(rerank_path)
        if rerank_rows:
            row = dict(rerank_rows[0])
            row["model_name"] = "poemetric_reranking"
            rows.append(row)
    return rows


def write_summary(dev_eval: Path, test_eval: Path, runs: list[dict[str, str]], selected_source: str) -> None:
    output_runs = list(runs)
    rerank_dev_prediction = DPO_RERANK_ROOT / "predictions" / "dev_reranked.txt"
    rerank_test_prediction = DPO_RERANK_ROOT / "predictions" / "test_reranked.txt"
    if rerank_dev_prediction.exists() and rerank_test_prediction.exists():
        output_runs = [run for run in output_runs if run["name"] != "poemetric_reranking"]
        output_runs.append(
            {
                "name": "poemetric_reranking",
                "checkpoint": str(DPO_RERANK_ROOT / "predictions"),
                "dev_prediction": str(rerank_dev_prediction),
                "test_prediction": str(rerank_test_prediction),
            }
        )
    ...
    definitions = {
        ...
        "poemetric_reranking": "reference-free POEMetric reranking on top of the best DPO checkpoint",
    }
    for name, desc in definitions.items():
        checkpoint = run_meta.get(name, {}).get("checkpoint", "")
        if name == "poemetric_reranking":
            checkpoint = str(DPO_RERANK_ROOT / "predictions")
        lines.append(f"| `{name}` | {desc} | `{checkpoint}` |")
    ...
    for row in rows_with_poemetric_reranking(dev_eval, DPO_RERANK_DEV_SUMMARY):
        lines.append(
    ...
    for row in rows_with_poemetric_reranking(test_eval, DPO_RERANK_TEST_SUMMARY):
        lines.append(
            "| {name} | {chrf} | {sonnet} | {form} | {lexical} | {overall} | {theme} | {poemetric} |".format(
                name=row.get("model_name"),
                chrf=fmt(row.get("chrF")),
    ...
    (OUT_ROOT / "runs.json").write_text(json.dumps(output_runs, indent=2, ensure_ascii=False) + "\n", encoding="utf-8")
