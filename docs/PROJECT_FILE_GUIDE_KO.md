...
| 파일 | 역할 |
|---|---|
| `PROJECT_README_KO.md` | 프로젝트 빠른 안내 |
...
| `EVALUATION_METRICS_GUIDE.md` | 기존 metric script 사용 가이드 |
| `sonnet_data/README.md` | 데이터 split과 strict 497 정제 설명 |

## Reproducibility Scripts

| 파일 | 역할 |
|---|---|
| `run_sixway_sonnet_ablation.py` | 6개 실험 학습/생성/평가 통합 runner |
| `evaluate_sonnet_poemetric.py` | chrF, Sonnet-or-Not proxy, POEMetric proxy 평가 |
| `evaluate_sonnet_metrics.py` | BLEU, ROUGE-L, token-F1 등 더 넓은 metric 평가 |
| `generate_sonnet_checkpoint.py` | 저장된 checkpoint로 dev/test prediction 생성 |
| `train_lora_sft_from_checkpoint.py` | 기존 checkpoint에서 LoRA-SFT 수행 |
| `run_msk_sft_lora_dpo.py` | LoRA-DPO 기본 구현 |
| `run_msk_sft_lora_dpo_form_rhyme.py` | form/rhyme-aware LoRA-DPO 구현 |
