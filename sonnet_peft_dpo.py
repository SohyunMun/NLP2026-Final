'''
[Variation 5: PEFT + DPO]
소네트 생성 모델 - PEFT(LoRA + Prefix Tuning) + DPO 버전

실행:
  `python sonnet_peft_dpo.py --epochs 1 --batch_size 2`
'''
...
# Native Prefix Tuning module (Prepends continuous virtual tokens at the embedding layer)
...
class SonnetGPT(nn.Module):
  ...
  def forward(self, input_ids, attention_mask):
    if self.use_peft:
      ...
      # Attention mask 병합 (Prefix 부분은 마스킹 1로 채움)
      ...
      # Prefix 영역은 떼고 원래 시퀀스 길이만큼만 예측용으로 추출
...
def train(args):
  ...
  # 1. Policy 모델 (LoRA + Prefix Tuning 주입 및 학습 활성화)
  policy_model = SonnetGPT(args, use_peft=True).to(device)
  
  # 2. Reference 모델 (동결된 베이스라인 모델)
  ref_model = SonnetGPT(args, use_peft=False).to(device)
  ref_model.eval()
  for param in ref_model.parameters():
    param.requires_grad = False

  # LoRA + Prefix Tuning 가중치만 옵티마이저에 전달
  ...
  for epoch in range(args.epochs):
    ...
    for batch in tqdm(train_loader, desc=f'PEFT+DPO train-{epoch}', disable=TQDM_DISABLE):
    ...
    print(f"Epoch {epoch}: PEFT+DPO train loss :: {train_loss:.3f} | val loss :: {val_loss:.3f}")
...
def generate_submission_sonnets(args):
  ...
  model = SonnetGPT(saved['args'], use_peft=True)
  ...
  print("\n=============================================")
  print("Final Evaluation on Fixed Metric Suite (PEFT + DPO)")
  print("=============================================")
...
def get_args():
  ...
  parser.add_argument("--prefix_len", type=int, default=8) # Prefix Tuning의 가상 토큰 길이
...
if __name__ == "__main__":
  args = get_args()
  args.filepath = f'{args.epochs}-{args.lr}-sonnet-peft-dpo.pt'
